...
def main():
    ...
    filtered_average_salaries_by_industry = []
    for group in average_salaries_by_industry:
        if group.participant_count >= 10:
            filtered_average_salaries_by_industry.append(group)

    sorted_filtered_average_salaries_by_industry = sorted(filtered_average_salaries_by_industry, key=lambda x: x.average, reverse=True)
...
def group_by_attribute(objects, property):
    '''
        params:
            ...
            property: Str - whichever object propety we want to group by
        return:
